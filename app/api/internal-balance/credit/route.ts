...
function parseAmount(payload: AdminBalancePayload): bigint {
  ...
  try {
    ...
    console.error("[api:internal-balance:credit] Failed to parse amount:", error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const mutation = await creditInternalBalance({
      userId,
      walletAddress,
      amount,
    ...
    console.error("[api:internal-balance:credit] Failed to credit balance:", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to credit internal balance for this user",
      },
      { status: 400 },
    );
  }
