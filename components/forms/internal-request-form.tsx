...
export function InternalRequestForm(): JSX.Element {
  ...
  return (
    <section className="rounded-3xl border border-outline bg-surface shadow-card">
      <div className="flex flex-col gap-6 rounded-3xl bg-gradient-to-br from-backgroundAlt/40 to-surface/90 p-8 md:p-10">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <span className="pill bg-surfaceAlt text-foreground">{t("internalForm.badge")}</span>
            <h2 className="mt-4 text-3xl font-semibold text-foreground md:text-4xl display-title">
              {t("internalForm.title")}
            </h2>
            <p className="mt-2 max-w-2xl text-sm text-foregroundMuted md:text-base">
              {t("internalForm.description")}
            </p>
          </div>
          <div className="flex w-full max-w-xs flex-col rounded-2xl border border-outline bg-surfaceAlt p-4 text-xs text-foregroundMuted">
            <span className="font-semibold text-foreground">{t("internalForm.regulationTitle")}</span>
        ...
        <form className="grid gap-5 md:grid-cols-2" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold uppercase tracking-[0.24em] text-foregroundMuted">
              {t("internalForm.requester")}
            </label>
            <input
              className="rounded-2xl border border-outline bg-surface px-4 py-3 text-sm text-foreground outline-none transition hover:border-accent focus:border-accent focus:ring-2 focus:ring-accent/20"
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex h-full items-center gap-3 rounded-2xl border border-outline bg-surface px-4 py-3">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    form.priority === priority
                      ...
                      : "bg-surfaceAlt text-foreground hover:bg-accent/10",
                  )}
                >
          ...
          <div className="md:col-span-2 flex flex-col gap-2">
            ...
            <textarea
              className="min-h-[140px] resize-y rounded-2xl border border-outline bg-surface px-4 py-3 text-sm text-foreground outline-none transition hover:border-accent focus:border-accent focus:ring-2 focus:ring-accent/20"
          ...
          <div className="md:col-span-2 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="text-xs text-foregroundMuted">{t("internalForm.helper")}</div>
